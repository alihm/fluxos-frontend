...
export const getComputedNavLinkToProp = computed(() => link => {
  ...
  if (link.to) {
    // If to is a string starting with '/', treat it as a path
    if (typeof link.to === 'string' && link.to.startsWith('/')) {
      props.to = link.to
    }

    // If to is a string and not a path, treat it as a route name
    else if (typeof link.to === 'string') {
      props.to = { name: link.to }
    }

    // If to is an object, use it directly
    else {
      props.to = link.to
    }
  } else {
...
export const getDynamicI18nProps = (key, tag = 'span') => {
  ...
  const unwrappedKey = unref(key)

  if (!layoutConfig.app.i18n.enable)
    return {}
  
  return {
    keypath: unwrappedKey,
    tag,
    scope: 'global',
  }
}
